...
public class JacobiService {
  public CommonResponse<Map<String, Object>> solve(LinearSysReq req) {
    ...
    for (; iter < maxIter; iter++) {
      ...
      double err = 0.0;
      for (int i = 0; i < n; i++) {
        err = Math.max(err, Math.abs(xNew[i] - x[i]));
      }
      if (logSteps) {
        Step step = new Step();
        step.setItrs(iter);
        step.setX(Arrays.copyOf(xNew, n));
        step.setErr(err);
    ...
    result.put("solution", Arrays.copyOf(xNew, n));
    res.setRes(result);
    res.setItrs(iter + 1);
    ...
    res.setMessage(converged ? "Converged successfully"
        : (res.getMessage() != null ? res.getMessage() : "Did not converge"));
    return res;
